                        ...
                        <div class="cart-item flex"><a href="product-item.php?id=${item.id}">
                            ...
                            <div class="cart-details flex flex-column justify-content-between p-2">
                                <div class="cart-price flex justify-content-between align-items-center">
                                    ...
                                    <div class="flex"><p class="pe-2"></p><h5><span class="naira">&#8358;</span> ${total.toLocaleString()}</span></h5></div>
                                </div>
                                <div class="flex flex-wrap justify-content-between">
                                    <button onclick="removeItem(${item.id})" class=" btn-sm remove-item border border-dark rounded-2 p-1"><i class="fas fa-trash"></i><span>Remove Item</span></button>
                                    <div class="flex align-items-center flex-wrap">
                                        <p><span>&#8358;</span>${item.price}.00 x</p>
                                        <input type="number" data-id="${item.id}" placeholder="${item.quantity}" class='updateQuantity modify-cart ms-2 mt-1'/>
...
document.addEventListener("input", function (e) {
  if (e.target.classList.contains("updateQuantity")) {
    ...
    let updateValue = parseFloat(e.target.value) || 0;
    let productId = e.target.dataset.id;

    let cart = JSON.parse(localStorage.getItem("cart")) || [];

    const item = cart.find((p) => parseFloat(p.id) == parseFloat(productId));
    if (!item) return;
    ...
    if (item.quantity <= 0) {
      cart = cart.filter((p) => parseFloat(p.id) !== parseFloat(productId));
    }
