            ...
            <div class="cart-item flex"><a href="product-item.html?id=${item.id}">
                            ...
                            <div class="cart-details flex flex-column justify-content-between p-2">
                                <div class="cart-price flex justify-content-between align-items-center">
                                    <h3>${item.name}</h3>
                                    <div class="flex"><h5><span class="naira">&#8358;</span> ${cleanPrice.toLocaleString()}</span></h5></div>
                                </div></a>
                                <div class="remove-item text-center">
                                    <button onclick="removeItem(${item.id})" class=" btn-sm border border-dark rounded-2 p-1"><i class="fas fa-trash"></i> Remove From Favorite</button>
                                </div>
                            </div>
                        </div>
        `;
